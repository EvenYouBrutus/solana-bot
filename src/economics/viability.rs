...
pub fn break_even_calculator(input: &BreakEvenInputs) -> Result<BreakEvenResult, ViabilityError> {
    ...
    for (value, name) in [
        ...
        if value < Decimal::ZERO {
            return Err(ViabilityError::Negative(name));
        }
    }
    ...
    let round_trip_cost_usd =
        dec!(2) * (fill_cost + input.avg_priority_fee_usd) + expected_failed_tx_cost_usd;
    ...
    let required_win_rate =
        (loss + cost_pct) / (loss * (Decimal::ONE + input.assumed_win_loss_ratio));
    let required_avg_win_pct = (loss + cost_pct) / input.assumed_win_loss_ratio + cost_pct;
    Ok(BreakEvenResult {
        round_trip_cost_usd,
        round_trip_cost_pct_of_position: cost_pct,
        required_win_rate_at_given_rr: required_win_rate,
        required_avg_win_pct,
        expected_failed_tx_cost_usd,
    })
}

fn positive(value: Decimal, name: &'static str) -> Result<(), ViabilityError> {
    if value > Decimal::ZERO {
        Ok(())
    } else {
        Err(ViabilityError::NonPositive(name))
    }
}
fn probability(value: Decimal, name: &'static str) -> Result<(), ViabilityError> {
    if value >= Decimal::ZERO && value <= Decimal::ONE {
        Ok(())
    } else {
        Err(ViabilityError::ProbabilityOutOfRange(name))
    }
}
...
mod tests {
    ...
    #[test]
    fn includes_all_expected_costs_and_solves_break_even() {
        let r = break_even_calculator(&input()).unwrap();
        assert_eq!(r.round_trip_cost_usd, dec!(0.3044));
        assert_eq!(r.round_trip_cost_pct_of_position, dec!(3.04400));
        assert_eq!(r.required_win_rate_at_given_rr.round_dp(4), dec!(0.4348));
    }
    #[test]
    fn rejects_adversarial_invalid_probability() {
        let mut i = input();
        i.failed_tx_rate = dec!(1.01);
        assert_eq!(
            break_even_calculator(&i).unwrap_err(),
            ViabilityError::ProbabilityOutOfRange("failed_tx_rate")
        );
    }
}
