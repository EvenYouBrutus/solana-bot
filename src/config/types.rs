...
pub struct RpcConfig {
    ...
    #[serde(default)] pub websocket_endpoints: Vec<String>,
    #[serde(default = "default_stale")] pub max_data_age_secs: i64,
    #[serde(default = "default_timeout")] pub request_timeout_secs: u64,
    /// Bounded retry passes over every endpoint before giving up. Never infinite.
    #[serde(default = "default_rpc_attempts")] pub max_attempts: u32,
    ...
    #[serde(default = "default_unknown_after")] pub unknown_after_secs: i64,
}
#[derive(Debug, Clone, Deserialize)]
...
pub struct EconomicsConfig {
    ...
    #[serde(default)] pub sol_price_usd: Option<Decimal>,
}
#[derive(Debug, Clone, Deserialize)]
pub struct RiskConfig {
    ...
    #[serde(default = "default_max_failures")] pub max_consecutive_failures: u32,
}
#[derive(Debug, Clone, Deserialize)]
pub struct ExecutionConfig {
    ...
    #[serde(default = "default_max_fee")] pub max_fee_lamports: u64,
    /// Reject quotes whose price impact exceeds this ceiling, independently
    /// of the slippage tolerance.
    #[serde(default = "default_price_impact")] pub max_price_impact_bps: u32,
    ...
    #[serde(default = "default_confirm_timeout")] pub confirm_timeout_secs: u64,
    #[serde(default = "default_confirm_poll_ms")] pub confirm_poll_ms: u64,
    #[serde(default)] pub live_signer_env: Option<String>,
    #[serde(default)] pub allowed_program_ids: Vec<String>,
}
#[derive(Debug, Clone, Copy, Deserialize, PartialEq, Eq)]
#[serde(rename_all = "lowercase")]
...
pub struct RuntimeConfig {
    #[serde(default)] pub signal_feed_path: Option<String>,
    #[serde(default = "default_poll")] pub poll_interval_secs: u64,
    #[serde(default = "default_paper_haircut")] pub paper_fill_haircut_bps: u32,
    ...
    #[serde(default = "default_reconcile_interval")] pub reconcile_interval_secs: u64,
}
#[derive(Debug, Clone, Deserialize)]
...
fn default_rpc_attempts() -> u32 { 2 }
fn default_unknown_after() -> i64 { 180 }
fn default_max_failures() -> u32 { 3 }
fn default_max_fee() -> u64 { 500_000 }
fn default_price_impact() -> u32 { 300 }
fn default_confirm_timeout() -> u64 { 90 }
fn default_confirm_poll_ms() -> u64 { 500 }
...
impl Default for RuntimeConfig {
    fn default() -> Self { Self { signal_feed_path: None, poll_interval_secs: default_poll(), paper_fill_haircut_bps: default_paper_haircut(), reconcile_interval_secs: default_reconcile_interval() } }
}
impl Config {
    pub fn validate(&self) -> Result<(), ConfigError> {
        ...
        if self.rpc.max_attempts == 0 { return Err(ConfigError::Invalid("rpc.max_attempts must be at least 1".into())); }
        ...
        if self.risk.max_consecutive_failures == 0 { return Err(ConfigError::Invalid("max_consecutive_failures must be positive".into())); }
        ...
        if let Some(price) = self.economics.sol_price_usd { if price <= Decimal::ZERO { return Err(ConfigError::Invalid("sol_price_usd must be positive when provided".into())); } }
        if self.execution.max_fee_lamports < self.execution.priority_fee_lamports.saturating_add(5_000) { return Err(ConfigError::Invalid("max_fee_lamports must cover base fee plus configured priority fee".into())); }
        if self.execution.max_price_impact_bps == 0 || self.execution.max_price_impact_bps > 10_000 { return Err(ConfigError::Invalid("max_price_impact_bps must be in (0,10000]".into())); }
        if self.execution.confirm_timeout_secs < 5 { return Err(ConfigError::Invalid("confirm_timeout_secs must be at least 5".into())); }
        if self.execution.confirm_poll_ms == 0 { return Err(ConfigError::Invalid("confirm_poll_ms must be positive".into())); }
        ...
        if self.mode == Mode::Live && self.runtime.reconcile_interval_secs == 0 { return Err(ConfigError::Invalid("live mode requires periodic on-chain position reconciliation".into())); }
        ...
        if self.strategy.stop_loss_pct <= Decimal::ZERO || self.strategy.take_profit_pct <= Decimal::ZERO || self.strategy.trailing_stop_pct < Decimal::ZERO { return Err(ConfigError::Invalid("stop-loss and take-profit must be positive; trailing stop non-negative".into())); }
        Ok(())
    }
}
...
"#;
    ...
    #[test]
    fn defaults_are_conservative_and_valid() { assert!(config().validate().is_ok()); }
    #[test]
    ...
    fn live_mode_still_requires_signer_and_allowlist() {
        ...
        assert!(c.validate().is_err());
        c.execution.allowed_program_ids = vec!["JUP6LkbZbjS1jKKwapdHNy74zcZ3tLUZoi5QNyVTaV4".into()];
