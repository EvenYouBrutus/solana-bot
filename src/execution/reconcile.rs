...
fn token_deltas(meta: &Value) -> Vec<(u64, String, i128)> {
    let parse = |key: &str| -> Vec<(u64, String, u64)> {
        meta[key].as_array().map(|arr| arr.iter().filter_map(|e| {
    ...
    keys.sort();
    keys.dedup();
    keys.into_iter().map(|(idx, mint)| {
        let before = pre.iter().find(|(i, m, _)| *i == idx && *m == mint).map(|(_, _, a)| *a).unwrap_or(0) as i128;
        let after = post.iter().find(|(i, m, _)| *i == idx && *m == mint).map(|(_, _, a)| *a).unwrap_or(0) as i128;
...
fn owner_for_index(meta: &Value) -> Vec<(u64, String)> {
    meta["postTokenBalances"].as_array().map(|arr| arr.iter().filter_map(|e| {
        let idx = e["accountIndex"].as_u64()?;
...
fn native_delta_for_owner(tx: &Value, owner: &str) -> Option<i128> {
    ...
    let idx = keys.iter().position(|k| k["pubkey"].as_str() == Some(owner))?;
    let meta = &tx["meta"];
...
pub fn parse_swap_transaction(tx: &Value, input_mint: &str, output_mint: &str, owner: &str, expected_input: u64) -> SwapOutcome {
    let meta = match tx.get("meta") { Some(m) if !m.is_null() => m, _ => return SwapOutcome::Unverifiable("missing transaction metadata".into()) };
    if let Some(err) = meta.get("err").filter(|e| !e.is_null()) { return SwapOutcome::Failed(err.clone()); }
    let Some(fee_lamports) = meta["fee"].as_u64() else { return SwapOutcome::Unverifiable("missing fee metadata".into()) };
    let owners = owner_for_index(meta);
    let deltas = token_deltas(meta);

    let delta_for = |mint: &str| -> i128 {
        deltas.iter().filter(|(idx, m, _)| *m == mint && owners.iter().any(|(oi, o)| oi == idx && o == owner)).map(|(_, _, d)| *d).sum()
    };
    ...
    let input_amount = if input_mint == WSOL_MINT {
        ...
        if d <= 0 { return SwapOutcome::Unverifiable(format!("no positive owner delta for input mint {input_mint}")); }
        match u64::try_from(d) { Ok(v) => v, Err(_) => return SwapOutcome::Unverifiable("input delta overflow".into()) }
    };

    let output_amount = if output_mint == WSOL_MINT {
        match native_delta_for_owner(tx, owner) {
            Some(d) if d > 0 => { let gross = d + fee_lamports as i128; match u64::try_from(gross) { Ok(v) => v, Err(_) => return SwapOutcome::Unverifiable("output delta overflow".into()) } }
            _ => return SwapOutcome::Unverifiable("no positive native delta for wrapped-SOL output".into()),
        }
    } else {
        let d = delta_for(output_mint);
        if d <= 0 { return SwapOutcome::Unverifiable(format!("no positive owner delta for output mint {output_mint}")); }
        match u64::try_from(d) { Ok(v) => v, Err(_) => return SwapOutcome::Unverifiable("output delta overflow".into()) }
    };

    SwapOutcome::Executed(ChainSwapOutcome { input_amount, output_amount, fee_lamports, block_time: tx["blockTime"].as_i64() })
}
...
mod tests {
    ...
    fn tb(idx: u64, mint: &str, amount: u64, owner: Option<&str>) -> Value {
        ...
        if let Some(o) = owner { e["owner"] = json!(o); }
        e
    }

    #[test]
    fn extracts_actual_amounts_and_fees() {
        let tx = tx_json(10_000, json!([tb(1, TOKEN, 0, Some(OWNER))]), json!([tb(1, TOKEN, 5_000_000, Some(OWNER))]), None, 990_000);
        let out = parse_swap_transaction(&tx, WSOL_MINT, TOKEN, OWNER, 1_000_000_000);
        assert_eq!(out, SwapOutcome::Executed(ChainSwapOutcome { input_amount: 1_000_000_000, output_amount: 5_000_000, fee_lamports: 10_000, block_time: Some(1_700_000_000) }));
    }
    #[test]
    fn wsol_output_comes_from_native_delta_plus_fee() {
        let tx = tx_json(10_000, json!([tb(1, TOKEN, 5_000_000, Some(OWNER))]), json!([tb(1, TOKEN, 0, Some(OWNER))]), None, 1_090_000);
        let out = parse_swap_transaction(&tx, TOKEN, WSOL_MINT, OWNER, 5_000_000);
        match out { SwapOutcome::Executed(o) => { assert_eq!(o.output_amount, 90_000 + 10_000); assert_eq!(o.input_amount, 5_000_000); }, other => panic!("{other:?}") }
    }
    #[test]
    fn onchain_failure_is_distinct_from_unverifiable() {
        let tx = tx_json(5_000, json!([]), json!([]), Some(json!("SlippageToleranceExceeded")), 995_000);
        assert!(matches!(parse_swap_transaction(&tx, WSOL_MINT, TOKEN, OWNER, 1), SwapOutcome::Failed(_)));
    }
    #[test]
    fn missing_owner_delta_is_unverifiable_not_assumed() {
        let tx = tx_json(10_000, json!([]), json!([tb(1, TOKEN, 5_000_000, Some("SomeoneElse11111111111111111111111111111111"))]), None, 990_000);
        assert!(matches!(parse_swap_transaction(&tx, WSOL_MINT, TOKEN, OWNER, 1), SwapOutcome::Unverifiable(_)));
        let tx = tx_json(10_000, json!([]), json!([]), None, 990_000);
        assert!(matches!(parse_swap_transaction(&tx, WSOL_MINT, TOKEN, OWNER, 1), SwapOutcome::Unverifiable(_)));
    }
    #[test]
    fn unowned_balance_entries_without_owner_field_are_refused() {
        let tx = tx_json(10_000, json!([tb(1, TOKEN, 0, None)]), json!([tb(1, TOKEN, 5_000_000, None)]), None, 990_000);
        assert!(matches!(parse_swap_transaction(&tx, WSOL_MINT, TOKEN, OWNER, 1), SwapOutcome::Unverifiable(_)));
    }
    #[test]
