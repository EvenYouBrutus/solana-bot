...
pub fn units(amount: u64, decimals: u8) -> Result<Decimal, ExecutionError> {
    let d = rust_decimal::Decimal::from(
        10u64
            ...
            .ok_or_else(|| ExecutionError::Policy("decimals overflow".into()))?,
    );
...
mod tests {
    ...
    fn entry_pricing_uses_actual_output() {
        let (v, p) = ValueBasis::InputValueUsd(dec!(10))
            .price_fill(1_000_000_000, 9, 5_000_000, 6)
            .unwrap();
        assert_eq!(v, dec!(10));
        assert_eq!(p, dec!(2));
    }
    ...
    fn exit_pricing_uses_actual_base_output() {
        let (v, p) = ValueBasis::OutputUnitPriceUsd(dec!(150))
            .price_fill(5_000_000, 6, 1_500_000_000, 9)
            .unwrap();
        assert_eq!(v, dec!(225));
        assert_eq!(p, dec!(45));
    }
    ...
    fn zero_legs_are_refused() {
        assert!(ValueBasis::InputValueUsd(dec!(10))
            .price_fill(1, 9, 0, 6)
            .is_err());
        assert!(ValueBasis::OutputUnitPriceUsd(dec!(1))
            .price_fill(0, 6, 1, 9)
            .is_err());
    }
    #[test]
